...
import { getCategorySummaries } from "../../../lib/product";
import { getApprovedCustomerReviews, getApprovedCustomerReviewSummary } from "../../../lib/product-social-server";
...
export default async function HomeDeferredSections() {
  ...
  const visibleCategories = Array.isArray(categories) ? categories.slice(0, 12) : [];
  const hasReviews = customerReviews.length > 0 || Number(reviewSummary?.reviewCount || 0) > 0;
      ...
      {visibleCategories.length > 0 ? (
        <section className="bg-[linear-gradient(180deg,#f6ede2_0%,#fbf7f0_100%)] py-14">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            />
            <CategoryTilesClient categories={visibleCategories} />
          </div>
        </section>
      ) : null}
