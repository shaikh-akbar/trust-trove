...
import { PackageCheck, Save, Search, Sparkles, X } from "lucide-react";
...
export default function AdminProductsClient({ initialProducts = [], statusOptions = [] }) {
  const [products, setProducts] = useState(initialProducts);
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState("all");
  const [selectedId, setSelectedId] = useState(initialProducts[0]?.id || "");
  const [draft, setDraft] = useState(initialProducts[0] || null);
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,0.88fr)_minmax(370px,0.72fr)]">
        ...
        <div className="rounded-[2rem] border border-[var(--line)] bg-white p-4 shadow-[0_20px_60px_-48px_rgba(66,72,121,0.24)] sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="rounded-[1.4rem] border border-[var(--line)] bg-[var(--surface-soft)] px-4 py-3 text-right">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Loaded products</p>
              <p className="mt-1 text-xl font-black tracking-tight text-[var(--brand-navy)]">{products.length}</p>
            </div>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {filteredProducts.length > 0 ? (
            ...
            <div className="md:col-span-2 rounded-[2rem] border border-dashed border-[var(--line)] bg-white px-6 py-14 text-center">
              <p className="text-lg font-black text-slate-900">{buildEmptyMessages(query, filter)}</p>
      ...
      <section className="rounded-[2rem] border border-[var(--line)] bg-white p-5 shadow-[0_20px_60px_-48px_rgba(66,72,121,0.24)] xl:sticky xl:top-6 xl:self-start">
        {draft ? (
          <form onSubmit={handleSave} className="space-y-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.24em] text-slate-400">Product editor</p>
                <h3 className="mt-2 text-2xl font-black tracking-[-0.04em] text-slate-950">{draft.title}</h3>
                <p className="mt-2 text-sm text-slate-500">Update homepage featured state, status, and SEO copy from one place.</p>
              </div>
              {draft.is_featured ? (
                <span className="inline-flex items-center gap-2 rounded-full bg-[var(--brand-navy)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white">
                  <Sparkles size={12} />
                  Featured
                </span>
              ) : null}
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--brand-navy)] px-6 py-3 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:bg-[#313b78] disabled:cursor-wait disabled:opacity-70"
            >
              <Save size={16} />
          ...
          <div className="flex min-h-[28rem] items-center justify-center rounded-[1.5rem] border border-dashed border-[var(--line)] bg-[var(--surface-soft)] px-6 text-center">
            <div>
              <p className="text-lg font-black text-slate-900">Select a product to edit.</p>
              <p className="mt-2 text-sm text-slate-500">Featured state, status, SEO copy, and supplier fields will appear here.</p>
            </div>
          </div>
        )}
      </section>
