...
async function getAdminSearchMatchedProductIds(search) {
  const normalizedSearch = String(search || "").trim();
  ...
  const pattern = `%${escapeIlikeValue(normalizedSearch)}%`;

  const [{ data: productMatches, error: productError }, { data: variantMatches, error: variantError }] =
    await Promise.all([
      supabase
        .from("products")
        .select("id")
        .or(
          [
            ...
            `supplier_product_code.ilike.${pattern}`,
          ].join(",")
        ),
      supabase
        .from("variants")
        .select("product_id")
        .or([`sku.ilike.${pattern}`, `supplier_sku.ilike.${pattern}`].join(",")),
    ]);
  ...
  return Array.from(
    new Set([
      ...(productMatches || []).map((item) => item.id).filter(Boolean),
      ...(variantMatches || []).map((item) => item.product_id).filter(Boolean),
    ])
  );
}

async function getFilteredAdminProductIds({ search = "", status = "all", category = "all" } = {}) {
  const supabase = getSupabaseAdmin();
  const normalizedStatus = String(status || "all").trim().toLowerCase();
  const normalizedCategory = String(category || "all").trim();
  const searchMatchedIds = await getAdminSearchMatchedProductIds(search);

  if (Array.isArray(searchMatchedIds) && searchMatchedIds.length === 0) {
    return [];
  }
  ...
  if (Array.isArray(searchMatchedIds)) {
  ...
  return (data || [])
    .filter((product) => {
      ...
      return getPublicCategoryLabel(product) === normalizedCategory;
    })
...
export async function getAdminProductsPage({
  ...
  const [filteredProductIds, categories] = await Promise.all([
    getFilteredAdminProductIds({ search, status, category }),
  ...
  const pageFrom = (resolvedPage - 1) * safePageSize;
  const pageTo = pageFrom + safePageSize - 1;
  const pageProductIds = filteredProductIds.slice(pageFrom, pageTo + 1);
  ...
  return {
    items: (data || []).map(formatAdminProduct),
    page: resolvedPage,
    pageSize: safePageSize,
    total,
    totalPages,
