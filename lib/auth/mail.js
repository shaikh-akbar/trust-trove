...
function buildOrderItemsMarkup(items = []) {
  return (Array.isArray(items) ? items : [])
    .map((item) => {
      const supplierLabel = formatLabel(item?.supplierName, "unknown supplier");
      const optionLabel = item?.optionLabel ? ` (${escapeHtml(item.optionLabel)})` : "";
      const quantity = Number(item?.quantity || 0);
      const lineTotal = formatPrice(item?.lineTotal || 0);

      return `
        <tr>
          <td style="padding:0 0 12px;font-size:14px;line-height:22px;color:#0f172a;">
            <strong>${escapeHtml(item?.title || "Untitled product")}</strong>${optionLabel}<br />
            <span style="color:#64748b;font-size:12px;">Supplier: ${escapeHtml(supplierLabel)} | Qty: ${escapeHtml(quantity || 0)}</span>
          </td>
          <td align="right" style="padding:0 0 12px;font-size:14px;font-weight:700;color:#0f172a;">
...
export async function sendSupplierOrderNotification({
  ...
  const supplierList = Array.isArray(suppliers) ? suppliers.filter(Boolean) : [];
  const supplierLabel = supplierList.length ? supplierList.map((value) => formatLabel(value, value)).join(", ") : "supplier";
  const paymentLabel = formatLabel(paymentType, "online");
  const customerName = customer?.fullName?.trim() || "Customer";
  ...
  const customerAddress = [
    ...
    .map((value) => normalizeTextPart(value))
    .filter(Boolean)
    .join(", ");

  return sendEmail({
    to,
    subject: subjectLine || `New GoModexa order ${orderNumber} for ${supplierLabel}`,
    html: `
      <div style="margin:0;padding:24px 12px;background:#f8fafc;font-family:Arial,sans-serif;color:#0f172a;">
        <div style="max-width:720px;margin:0 auto;border:1px solid #dbe4f0;border-radius:24px;background:#ffffff;overflow:hidden;">
          <div style="padding:24px 28px;background:#161f66;color:#ffffff;">
            <div style="font-size:11px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:rgba(255,255,255,0.72);">
              Supplier Order Alert
            </div>
            <h1 style="margin:10px 0 6px;font-size:24px;line-height:30px;">New order for ${escapeHtml(supplierLabel)}</h1>
            <p style="margin:0;font-size:14px;line-height:22px;color:rgba(255,255,255,0.82);">
              Order <strong>${escapeHtml(orderNumber)}</strong> has been confirmed.
            </p>
          </div>
          ...
          <div style="padding:24px 28px;">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 20px;border-collapse:collapse;">
              <tr>
                <td style="padding:0 0 10px;font-size:13px;color:#64748b;">Payment type</td>
                <td align="right" style="padding:0 0 10px;font-size:14px;font-weight:700;color:#0f172a;text-transform:capitalize;">${escapeHtml(paymentLabel)}</td>
              </tr>
              <tr>
                <td style="padding:0;font-size:13px;color:#64748b;">Order total</td>
                <td align="right" style="padding:0;font-size:18px;font-weight:800;color:#161f66;">${escapeHtml(formatPrice(totalAmount))}</td>
            ...
            <div style="margin:0 0 20px;padding:18px;border:1px solid #e2e8f0;border-radius:18px;background:#f8fafc;">
              <p style="margin:0 0 8px;font-size:12px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:#64748b;">Customer</p>
              <p style="margin:0 0 6px;font-size:14px;font-weight:700;color:#0f172a;">${escapeHtml(customerName)}</p>
              <p style="margin:0 0 4px;font-size:14px;line-height:22px;color:#475569;">Phone: ${escapeHtml(customerPhone)}</p>
              <p style="margin:0 0 4px;font-size:14px;line-height:22px;color:#475569;">Email: ${escapeHtml(customerEmail)}</p>
              <p style="margin:0;font-size:14px;line-height:22px;color:#475569;">Address: ${escapeHtml(customerAddress || "-")}</p>
            </div>

            <div style="padding:18px;border:1px solid #e2e8f0;border-radius:18px;background:#ffffff;">
              <p style="margin:0 0 12px;font-size:12px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:#64748b;">Items</p>
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
                ${buildOrderItemsMarkup(items)}
              </table>
            </div>
          </div>
        </div>
