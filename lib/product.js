...
import { buildCategorySummary, formatCategoryLabel, slugifyCategory } from './storefront';
...
function applyCategoryFilter(query, categoryTitle) {
  if (!categoryTitle) {
    return query;
  }

  const escaped = escapeFilterValue(categoryTitle);
  return query.or(`category.eq.${escaped},product_type.eq.${escaped}`);
}
...
async function fetchProductsPageLive({ page = 1, pageSize = 60, categoryTitle = null } = {}) {
  const safePage = Math.max(1, Number(page || 1));
  const safePageSize = Math.max(1, Math.min(120, Number(pageSize || 60)));
  const from = (safePage - 1) * safePageSize;
  const to = from + safePageSize - 1;

  const { data, error, count } = await applyCategoryFilter(
    supabase
      .from('products')
      .select(
        `
...
export async function getProductsPage({ page = 1, pageSize = 60, categoryTitle = null, forceFresh = false } = {}) {
  const safePage = Math.max(1, Number(page || 1));
  const safePageSize = Math.max(1, Math.min(120, Number(pageSize || 60)));
  ...
  return withCatalogCache(
    cacheKey,
    () => fetchProductsPageLive({ page: safePage, pageSize: safePageSize, categoryTitle }),
...
export async function getHomePageData({
  ...
  const homeData = await withCatalogCache(
    cacheKey,
    async () => {
      const [categories, customerReviews] = await Promise.all([
  ...
  if (!forceFresh && (!hasCategories || !hasFeaturedTabs)) {
