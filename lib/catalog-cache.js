...
const CATALOG_CACHE_PREFIX = "gomodex:catalog:v3";
...
export async function withCatalogCache(key, loader, { ttlSeconds = DEFAULT_TTL_SECONDS, forceFresh = false } = {}) {
  ...
  if (!forceFresh) {
    cachedEntry = await readCatalogCache(key, ttlSeconds);

    if (cachedEntry?.isFresh && cachedEntry.value != null) {
  ...
  try {
    ...
    if (cachedEntry?.value != null) {
      console.error("Catalog cache loader failed, serving stale entry:", error);
      return cachedEntry.value;
    }
