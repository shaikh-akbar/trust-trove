...
import { sendOrderUpdateEmail, sendSupplierOrderNotification } from "./auth/mail";
...
import {
  ...
  DEFAULT_SHIPPING_TAX_PERCENT,
} from "./supplier-pricing";

const RAYSKET_SUPPLIER_ALIASES = new Set(["raysket", "yasar"]);
const RAYSKET_PREPAID_DISCOUNT_PERCENT = 10;
const SUPPLIER_NOTIFICATION_RECIPIENTS = {
  raysket: "raysketofficial@gmail.com",
  wukusy: "akbar000785@gmail.com",
  gomodexa: "akbar000785@gmail.com",
};
...
function isRaysketEligibleItem(item) {
  const supplierName = normalizeSupplierName(item?.supplier_name);
  const vendorName = normalizeSupplierName(item?.vendor);
  const categoryValue = normalizeCategoryValue(item?.category);

  return (
    RAYSKET_SUPPLIER_ALIASES.has(supplierName) ||
    RAYSKET_SUPPLIER_ALIASES.has(vendorName) ||
    categoryValue.includes("ladies bag") ||
    categoryValue.includes("ladies bags") ||
    categoryValue.includes("lady bag") ||
    categoryValue.includes("women bag") ||
    categoryValue.includes("womens bag")
  );
}
...
function calculateRaysketPrepaidDiscount(items, paymentType) {
  if (normalizePaymentType(paymentType) !== "online") {
    return 0;
  }

  const eligibleSubtotal = roundCurrency(
    (Array.isArray(items) ? items : []).reduce((total, item) => {
      if (!isRaysketEligibleItem(item)) {
        return total;
      }

      return total + normalizeNumber(item?.price_selling, 0) * normalizeNumber(item?.quantity, 1);
    }, 0)
  );

  return roundCurrency(eligibleSubtotal * (RAYSKET_PREPAID_DISCOUNT_PERCENT / 100));
}
...
function sumItemTax(items) {
  return roundCurrency(
    items.reduce((total, item) => {
      const quantity = normalizeNumber(item?.quantity, 1);
      const taxableAmount = normalizeNumber(item?.price_selling, 0) * quantity;
      const gstPercent = normalizeNumber(item?.gst_percent, DEFAULT_GST_PERCENT) || DEFAULT_GST_PERCENT;
      return total + taxableAmount * (gstPercent / 100);
    }, 0)
  );
}

function sumFixedMargin(items, marginAmount = DEFAULT_MARGIN_AMOUNT) {
  return roundCurrency(
    items.reduce((total, item) => {
      const quantity = normalizeNumber(item?.quantity, 1);
      return total + roundCurrency(marginAmount) * quantity;
    }, 0)
  );
}
...
function formatSupplierLabel(value) {
  ...
  if (normalizedValue === "gomodexa") {
    return "GoModexa";
  }

  if (normalizedValue === "wukusy") {
    return "Wukusy";
  }

  if (normalizedValue === "raysket") {
    return "Raysket";
  }
...
function buildSupplierNotificationGroups(items) {
  const grouped = new Map();

  for (const item of Array.isArray(items) ? items : []) {
    const supplierName = normalizeSupplierName(item?.supplier_name);
    const recipient = SUPPLIER_NOTIFICATION_RECIPIENTS[supplierName];

    if (!recipient) {
      continue;
    }

    const existing = grouped.get(recipient) || { suppliers: new Set(), items: [] };
    existing.suppliers.add(supplierName);
    existing.items.push({
      title: item?.title || "Untitled product",
      optionLabel: item?.optionLabel || item?.option_label || "Default",
      quantity: normalizeNumber(item?.quantity, 1),
      lineTotal: roundCurrency(
        item?.line_total ?? normalizeNumber(item?.price_selling, 0) * normalizeNumber(item?.quantity, 1)
      ),
      supplierName: formatSupplierLabel(supplierName),
    });
    grouped.set(recipient, existing);
  }

  return Array.from(grouped.entries()).map(([recipient, value]) => ({
    recipient,
    suppliers: Array.from(value.suppliers),
    items: value.items,
  }));
}

async function sendSupplierOrderNotificationsIfPossible({
  orderNumber,
  totalAmount,
  paymentType,
  addressSnapshot,
  ...
  const groups = buildSupplierNotificationGroups(items);

  await Promise.all(
    groups.map((group) =>
      sendSupplierOrderNotification({
        to: group.recipient,
        suppliers: group.suppliers.map(formatSupplierLabel),
        orderNumber,
        totalAmount,
        paymentType,
...
async function getOrderItemsForSupplierNotifications(orderId) {
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase
    .from("order_items")
    .select(`
...
async function reconcileOrderPaymentState(order, { razorpayPaymentId, razorpaySignature, paymentStatus, paidAt }) {
  ...
  if (isPaid && !wasAlreadyPaid) {
    ...
    sendOrderEmailIfPossible({
      ...
      subjectLine: `GoModexa payment confirmed: ${order.order_number}`,
    });

    const supplierItems = await getOrderItemsForSupplierNotifications(order.id);
    await sendSupplierOrderNotificationsIfPossible({
      orderNumber: order.order_number,
      totalAmount: order.total_amount,
      paymentType: order.payment_type,
...
export async function getCheckoutSummary(userId, couponCode = "", paymentType = "online", postalCode = "") {
  ...
  const subtotal = roundCurrency(cartSnapshot.subtotal);
  ...
  const couponDiscountAmount = calculateDiscount(subtotal, coupon);
  const prepaidDiscountAmount = calculateRaysketPrepaidDiscount(cartSnapshot.items, normalizedPaymentType);
  const discountAmount = Math.min(subtotal, roundCurrency(couponDiscountAmount + prepaidDiscountAmount));
  ...
  const itemTaxAmount = sumItemTax(cartSnapshot.items);
  const shippingTaxAmount = roundCurrency(shippingAmount * (DEFAULT_SHIPPING_TAX_PERCENT / 100));
  const marginAmount = sumFixedMargin(cartSnapshot.items);
  ...
  try {
    ...
    if (isCodOrder) {
      ...
      sendOrderEmailIfPossible({
        ...
        subjectLine: `GoModexa order confirmed: ${orderNumber}`,
      });
