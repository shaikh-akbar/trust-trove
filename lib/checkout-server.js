...
function getBaseShippingAmount(subtotal) {
...
export async function getCheckoutSummary(userId, couponCode = "", paymentType = "online") {
  ...
  const { codAvailable, unavailableCodItems } = getCodAvailability(cartSnapshot.items);
  const baseShippingAmount = roundCurrency(getBaseShippingAmount(discountedSubtotal));
...
export async function createCheckoutOrder(userId, { addressId, couponCode = "", paymentType }) {
  ...
  const normalizedPaymentType = normalizePaymentType(paymentType);
  const summary = await getCheckoutSummary(userId, couponCode, normalizedPaymentType);
