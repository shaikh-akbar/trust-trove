...
export const DEFAULT_MARGIN_AMOUNT = 100;
...
export function calculateSupplierDisplayPrice({
  ...
  // Wukusy merchant totals currently apply a fixed 18% item tax in the
  // supplier-side breakup, so storefront pricing mirrors that total and then
  // adds our fixed margin.
  const resolvedGstPercent = DEFAULT_GST_PERCENT;
...
export function resolveEffectiveSellingPrice({
  ...
  const computedPricing = calculateSupplierDisplayPrice({
    costPrice,
    gstPercent,
    weightGrams,
    marginAmount,
  ...
  return Math.max(0, computedPricing.displayPriceFinal);
}
