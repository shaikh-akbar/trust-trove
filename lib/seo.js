...
export function hasIndexableProductPageSignals(product) {
  ...
  const hasInventory = Array.isArray(product?.variants)
...
export function buildCategoryMetaTitle(category, { page = 1 } = {}) {
  ...
  const baseTitle = `${title} Online in India | Buy ${title}`;
...
export function buildCategoryMetaDescription(category, { page = 1 } = {}) {
  ...
  const baseDescription = trimMetaDescription(
    `Browse ${title} products online in India with curated picks, easier discovery, and practical shopping support on GoModexa.`
  );
...
export function buildCollectionMetadata({
  ...
  const shouldIndex = !normalizedQuery && normalizedPage === 1;
