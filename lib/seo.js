...
const DEFAULT_TITLE = "GoModexa Official Website | Premium Lifestyle Store in India";
const DEFAULT_DESCRIPTION =
  "Shop curated fashion, footwear, gifting, gadgets, and everyday lifestyle essentials on the official GoModexa website at gomodexa.com.";
const DEFAULT_KEYWORDS = [
  "GoModexa",
  "gomodexa",
  "gomodexa.com",
  "official GoModexa website",
...
export function buildCollectionMetadata({
  ...
  const shouldIndex =
    !normalizedQuery &&
    !hasFilters &&
    (normalizedPage === 1 || allowPaginatedIndex);
...
export function buildWebPageSchema({
  ...
  return {
    ...
    primaryImageOfPage: primaryImage
      ? {
          "@type": "ImageObject",
          url: primaryImage.startsWith("http") ? primaryImage : getSiteUrl(primaryImage),
        }
      : undefined,
    ...
    ...(totalPages > 1
      ? {
          mainEntity: {
            ...
            itemListElement: validItems.map((item, index) => ({
              "@type": "ListItem",
              position: index + 1 + Math.max(0, page - 1) * validItems.length,
...
export function buildBlogSchema({
  ...
  const validPosts = posts.filter((post) => post?.title && post?.path);
  ...
  return {
    ...
    blogPost: validPosts.map((post) => ({
      "@type": "BlogPosting",
      headline: post.title,
      url: getSiteUrl(post.path),
      datePublished: post.publishedAt || undefined,
      dateModified: post.updatedAt || post.publishedAt || undefined,
      image: post.image
        ? [post.image.startsWith("http") ? post.image : getSiteUrl(post.image)]
        : undefined,
